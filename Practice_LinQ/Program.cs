...
namespace Practice_LinQ
{
    ...
    public class Program
    {
        ...
        static void Main()
        {
            ...
            var result = debtors.Where(d => d.Email.Contains("rhyta.com") || d.Email.Contains("dayrep.com"));
            MyLoop(result);
            ...
            var result1 = debtors.Where(d => (DateTime.Now.Year - d.BirthDay.Year) >= 26 && (DateTime.Now.Year - d.BirthDay.Year) < 36);
            MyLoop(result1);
            ...
            var result2 = debtors.Where(d => (d.Debt <= 5000));
            MyLoop(result2);
            ...
            var result3 = debtors.Where(d => (d.FullName.Length > 18 && d.Phone.IndexOf("7") != d.Phone.LastIndexOf("7")));
            MyLoop(result3);
            ...
            var result4 = debtors.Where(d => (d.BirthDay.Month == 12 || d.BirthDay.Month == 1 || d.BirthDay.Month == 2));
            MyLoop(result4);
            ...
            var result5 = debtors.Where(d => (d.Debt > debtors.Average(d => d.Debt)))
                .OrderByDescending(d => d.Debt)
                .OrderBy(d => d.FullName[d.FullName.IndexOf(".") + 2]);
            MyLoop(result5);
            ...
            var result6 = debtors.Where(d => d.Phone.IndexOf("8") == -1);
            double allDebt = result.Sum(d => d.Debt);
            string surname = ""; int index;
            foreach (var item in result6)
            {
                index = item.FullName.IndexOf(". ");
                if (index >= 0 && index < item.FullName.Length - 1)
                {
                    surname = item.FullName.Substring(index + 2);
                }
                Console.WriteLine($"{surname}  {DateTime.Now.Year - item.BirthDay.Year}");
            }
            ...
            var result9 = debtors.OrderByDescending(d => d.Debt).Take(5).OrderBy(d => d.FullName);
            MyLoop(result9);
            ...
            double allDebt1 = debtors.Sum(d => d.Debt);
            Console.WriteLine($"All Debt : {allDebt1}");
            ...
            var result11 = debtors.Where(d => d.BirthDay.Year < 1941).OrderBy(d => d.FullName);
            MyLoop(result11);
            ...
            var result12 = debtors
            .Where(debtor => HasRepeatingDigits(debtor.Phone.Replace("-", "")));
            MyLoop(result12);
            ...
            decimal monthlyPayment = 500;

            var result13 = debtors.Where(d =>
            {
                decimal remDebt = d.Debt;
                int months = 0;
                while (remDebt > 0)
                {
                    remDebt -= monthlyPayment;
                    months++;
                }

                return DateTime.Today.AddMonths(months) >= DateTime.Today;
            });
            ...
            var result14 = debtors.Where(debtor => debtor.FullName.ToLower().Intersect("smile").Count() == 5);
            MyLoop(result14);

        }

        static bool HasRepeatingDigits(string phoneNumber)
        {
        ...
        public static void MyLoop(IEnumerable<Debtor> list)
        {
            int id = 0;
            Console.WriteLine("\t\t\t = = = D E B T O R  L I S T = = = \n\n\n");
            foreach (var debtor in list)
            {
                Console.WriteLine($"{++id}) {debtor}\n");
            }
        }
    }
}
